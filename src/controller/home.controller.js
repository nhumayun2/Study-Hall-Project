 ...
 */
export const getLocationOwnerHomepage = async (user) => {
...
// ====================================================================
// --- NEW: PUBLIC CATEGORY CONTROLLER ---
// ====================================================================
...
export const getPublicCategories = catchAsync(async (req, res) => {
  ...
  // 2. Get the counts of all active session offers, grouped by category
  const sessionCounts = await Session.aggregate([
    { $match: { status: "Active", type: "Offer" } },
  ...
  // 4. Merge the counts into the category list
  const categoriesWithCounts = categories.map((category) => ({
    ...category,
    sessionCount: countMap.get(category._id.toString()) || 0, // Default to 0 if no sessions
  }));
